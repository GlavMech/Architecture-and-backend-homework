...
def create_app():
    ...
    def counts():
        ...
        if team != "":
            # Получаем коллекцию для конкретной команды
            collection = client.get_database()[team]

            # Запрос к коллекции
            results = collection.find({"edate": edate})
            ed_count = results.count()  # Получаем количество документов
    ...
    def sgreport():
        ...
        team = t.strip()  # Убираем пробелы по краям
        print(team)
        ...
        response = requests.get(f"https://api.blabla.com/event_listings.php?token=blabla&eventid={skyid}")
        ...
        if team in ["Royals", "Pirates", "Reds", "Cubs", "Guardians"]:
            broaddf["floor_value"] = broaddf["notes"].apply(
                lambda x: str(x).split(" -ticketevolution")[0].split("BG:")[1]
            )

        if team == "blank":
            broaddf["section_match"] = broaddf["section"].apply(lambda x: f"Section {x[0:3]} {x[4:]}")
            print(broaddf)
        elif team in [
            "Athletics",
            "Cubs",
            ...
            broaddf["section_match"] = broaddf["section"].apply(lambda x: f"Section {x}")

        broaddf["row_match"] = broaddf["row"].apply(lambda x: f"Row {x}")
        skyseatnumdf = broaddf["seatnumbers"].str.split(",", expand=True)
        skyseatnumarr = skyseatnumdf.to_numpy(dtype=float, na_value=np.nan)
        skyminseatarr = [np.nanmin(m) for m in skyseatnumarr]
        skymaxseatarr = [np.nanmax(m) for m in skyseatnumarr]
        ...
        def matchfunction(x):
            ...
            # Преобразование sgseatmin и sgseatmax в целые числа, если они не равны "NA"
            sgseatmin = 0 if sgseatmin == "NA" else int(sgseatmin)
            sgseatmax = 0 if sgseatmax == "NA" else int(sgseatmax)

            sbrow = x["row_match"]
            sgrow = x["ROW_NAME"]

            if sbrow == sgrow:
                if (sgseatmin >= skyseatmin) and (sgseatmax <= skyseatmax):
                    return "Y"
                else:
                    return "N"
            else:
        ...
        def bpricegen(x):
            ...
            if ownlist == "N":
                # Проверка на "NA" для FACE_VALUE
                x_facevalue = 0 if x["FACE_VALUE"] == "NA" else float(x["FACE_VALUE"])
                newbprice = x_facevalue * 1.2
                return round(newbprice, 2)  # округление до двух знаков после запятой
            else:
                return float(listprice)  # преобразование в float для согласованности
        ...
        def lowest(x):
            ...
            if own == "Y":
                if (face_value == lowest_comp_price) and (size > 1):
                    return listprice, "NO CHANGES"

                elif (difference > 0) and (suggestedbprice < fv) and (size > 1):
                    return fv, "BFLOOR"

                elif (difference > 0) and (size > 1):
    ...
    def slackalert(inid: int, tokenkey=None):
        token = request.headers.get("TOKEN")
        if token == tokenkey:
            print(inid)
